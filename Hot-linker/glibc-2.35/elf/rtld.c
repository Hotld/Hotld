...
#endif
    smart_dynamic_loader_operations();
    if(dl_start_time){
      _exit(1);
    }
        ...
        ElfW(auxv_t) * auxv)
{
 ...
 information for the program.  */
  ...
  /* If the current libname is different from the SONAME, add the
     latter as well.  */
  if (GL(dl_rtld_map).l_info[DT_SONAME] != NULL && strcmp(GL(dl_rtld_map).l_libname->name,
                                                          (const char *)D_PTR(&GL(dl_rtld_map), l_info[DT_STRTAB]) + GL(dl_rtld_map).l_info[DT_SONAME]->d_un.d_val) != 0)
  {
 ...
 in _dl_map_object_from_fd.  */
    if (main_map->l_info[DT_SONAME] != NULL && (strcmp(((const char *)D_PTR(main_map, l_info[DT_STRTAB]) + main_map->l_info[DT_SONAME]->d_un.d_val), LIBC_SO) == 0))
      GL(dl_ns)
    [LM_ID_BASE].libc_map = main_map;
...
process_envvars(struct dl_main_state *state)
{
  ...
  while ((envline = _dl_next_ld_env_entry(&runp)) != NULL)
  {
...
#endif
      if (memcmp(envline, "PRINT_LIBS", 10) == 0)
      {
        _dl_printf("print libs\n");
        dl_print_libs = (envline[11] != '\0');
        break;
      }
      
      if (memcmp(envline, "SMALL_PAGE", 10) == 0)
      {
        _dl_printf("print libs\n");
        dl_small_page = (envline[11] != '\0');
        break;
      }
      //start_time
      if (memcmp(envline, "START_TIME", 10) == 0)
      {
        dl_start_time = (envline[11] != '\0');
        break;
      }
    ...
    case 11:
      ...
      if (memcmp(envline, "PRINT_DEBUG", 11) == 0)
      {
        dl_print_debug = (envline[12] != '\0');
        break;
      }

      if (memcmp(envline, "REWRITE_OFF", 11) == 0)
      {
        _dl_printf("print libs\n");
        dl_rewrite_indictCall = false;
        break;
      }

    case 12:
      /* The library search path.  */
      //_dl_printf("hot template path: NULL\n");
      ...
      if (memcmp(envline, "HOT_TEMPLATE", 12) == 0)
      {
        _dl_hot_template_path = &envline[13];
        break;
      }
